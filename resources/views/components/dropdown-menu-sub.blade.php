<div x-data="dropdownMenuSub" x-bind="root" role="menuitem" aria-haspopup="menu" {{$attributes}}>
    ...
    <button type="button" x-ref="subTrigger" x-bind="trigger" :class="{'bg-accent' : subOpen}"
        {{$dropdownMenuSubTrigger->attributes->class(["flex w-full cursor-default
    ...
    <template x-if="subOpen || subPreview">
        <div x-bind="content" {{$attributes->class(['z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover p-1
