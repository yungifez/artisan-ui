...
<div x-data='calendar(@json($selected), "{{$mode}}", @json($disabled) , @json($min), @json($max), @json($required))'
    ...
    <div class="flex items-center justify-between mb-3">
        <button x-bind="previousMonthTrigger" type="button"
            class="border dark:border-input inline-flex p-1 transition duration-100 ease-in-out rounded-lg focus:shadow-outline hover:bg-gray-100 dark:hover:opacity-10">
        ...
        <button x-bind="nextMonthTrigger" type="button"
            class="border dark:border-border inline-flex p-1 transition duration-100 ease-in-out rounded-lg focus:shadow-outline hover:bg-gray-100 dark:hover:opacity-10">
            <x-aui::angle-down class="inline-flex w-6 h-6 fill-foreground -rotate-90" />
        </button>
    </div>
