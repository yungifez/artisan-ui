...
<div x-data="select({{$attributes->has('multiple') ? 'true' : 'false'}}, {{$attributes->has('disabled') ? 'true' : 'false'}})"
    x-bind="root" class="{{$attributes->get('class')}} relative">
    ...
    <button x-bind="trigger" x-ref="select" type="button" @class(["flex min-h-10 w-full items-center justify-between
        rounded-md border bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground
        focus:outline-none focus:ring-2 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 ".(isset($trigger) ? $trigger?->attributes?->get('class') : '' )])>
...
<div class="w-6 pl-3 flex items-center">
    <x-aui::angle-down class="w-8/12 transition text-center fill-foreground" ::class="{'-rotate-180' : isOpen()}" />
</div>
</button>
