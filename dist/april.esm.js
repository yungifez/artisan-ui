...
var RangeModeHandler = class {
  constructor(selected, required, min, max) {
    ...
    if (selected?.from) {
      const from2 = this.createDateWithoutTime(selected.from);
      if (from2)
        this.dayClicked(from2);
    }
...
var calendar_default = (selected, mode, disabled, min, max, required, options = {}) => ({
  ...
  isDateInView(date) {
    ...
    if (!normalized)
      return false;
    const first2 = new Date(this.year, this.month, 1);
    const last = new Date(this.year, this.month + this.numberOfMonths, 0);
    return normalized >= first2 && normalized <= last;
  },
  moveMonth(amount) {
  ...
  calculateDays() {
    ...
    this.preBlankDaysInMonth = view.cells.filter((cell) => cell.outside && cell.date < first2).map((cell) => cell.day);
  ...
  isRangeMiddle(date) {
    const normalized = dateWithoutTime(date);
    const from2 = this.modeHandler?.value?.from;
    const to = this.modeHandler?.value?.to;
    return this.mode === "range" && !!from2 && !!to && normalized > from2 && normalized < to;
  },
  isRangeStart(date) {
...
var humanize = (value) => String(value).replace(/([a-z])([A-Z])/g, "$1 $2").replace(/[-_]/g, " ").replace(/^./, (letter) => letter.toUpperCase());
var escape = (value) => String(value).replace(/[&<>"']/g, (character) => ({
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  '"': "&quot;",
...
var chart_default = (data = [], config = {}, type = "bar", xKey = "", height = 240) => ({
  ...
  get categoryKey() {
    ...
    const first2 = this.data[0] ?? {};
    return Object.keys(first2).find((key) => !Number.isFinite(Number(first2[key]))) ?? Object.keys(first2)[0] ?? "";
  },
  get series() {
    ...
    const first2 = this.data[0] ?? {};
    return Object.keys(first2).filter((key) => key !== this.categoryKey && Number.isFinite(Number(first2[key]))).map((key, index) => this.seriesDefinition(key, index));
  },
  seriesDefinition(key, index) {
    const definition = this.config[key] ?? {};
    const theme = definition.theme ?? {};
    const color = definition.color ?? (document.documentElement.classList.contains("dark") ? theme.dark : theme.light) ?? `var(--chart-${index % 5 + 1})`;
    return {
      key,
  ...
  get geometryMarkup() {
    ...
    const areas = this.type === "area" ? this.series.map((series) => `<path d="${this.linePath(series, true)}" fill="${escape(series.color)}" fill-opacity="0.2" />`).join("") : "";
  ...
  get bars() {
    ...
    return this.data.flatMap((item, itemIndex) => this.series.map((series, seriesIndex) => {
      ...
      const height2 = value / this.maximum * this.plot.height;
      return {
        ...series,
        itemIndex,
        value,
        x: this.plot.left + itemIndex * groupWidth + gap / 2 + seriesIndex * width,
        y: this.plot.top + this.plot.height - height2,
  ...
  formatValue(value) {
    return new Intl.NumberFormat(void 0, { maximumFractionDigits: 1, notation: value >= 1e3 ? "compact" : "standard" }).format(value);
  },
  category(index) {
...
function getMarkRange($pos, type, attributes) {
  ...
  if (!findMarkInSet([...start.node.marks], type, attributes))
    return;
  ...
  if (isTextContent) {
    if (options.errorOnInvalidContent) {
      ...
      if (options.errorOnInvalidContent && hasInvalidContent)
        throw new Error("[tiptap error]: Invalid HTML content", { cause: /* @__PURE__ */ new Error(`Invalid element found: ${invalidContent}`) });
    }
...
var insertContentAt = (position, value, options) => ({ tr: tr2, dispatch, editor }) => {
  if (dispatch) {
    ...
    if (options.applyInputRules)
      tr2.setMeta("applyInputRules", {
        from: from2,
        text: newContent
      });
    if (options.applyPasteRules)
      tr2.setMeta("applyPasteRules", {
        from: from2,
        text: newContent
      });
  }
...
function splitExtensions(extensions2) {
  return {
    baseExtensions: extensions2.filter((extension) => extension.type === "extension"),
    nodeExtensions: extensions2.filter((extension) => extension.type === "node"),
    markExtensions: extensions2.filter((extension) => extension.type === "mark")
  };
}
...
function buildAttributeSpec(extensionAttribute) {
  ...
  const spec = {};
  if (!(extensionAttribute === null || extensionAttribute === void 0 || (_extensionAttribute$a = extensionAttribute.attribute) === null || _extensionAttribute$a === void 0 ? void 0 : _extensionAttribute$a.isRequired) && "default" in ((extensionAttribute === null || extensionAttribute === void 0 ? void 0 : extensionAttribute.attribute) || {}))
    spec.default = extensionAttribute.attribute.default;
  if ((extensionAttribute === null || extensionAttribute === void 0 || (_extensionAttribute$a2 = extensionAttribute.attribute) === null || _extensionAttribute$a2 === void 0 ? void 0 : _extensionAttribute$a2.validate) !== void 0)
...
var joinListBackwards = (tr2, listType) => {
  ...
  if (!(list.node.type === (nodeBefore === null || nodeBefore === void 0 ? void 0 : nodeBefore.type) && canJoin(tr2.doc, list.pos)))
    return true;
  if (!areListTypesCompatible(list.node.attrs.type, nodeBefore === null || nodeBefore === void 0 ? void 0 : nodeBefore.attrs.type))
    return true;
  tr2.join(list.pos);
  return true;
};
var joinListForwards = (tr2, listType) => {
  ...
  if (!(list.node.type === (nodeAfter === null || nodeAfter === void 0 ? void 0 : nodeAfter.type) && canJoin(tr2.doc, after)))
    return true;
  if (!areListTypesCompatible(list.node.attrs.type, nodeAfter === null || nodeAfter === void 0 ? void 0 : nodeAfter.attrs.type))
    return true;
  tr2.join(after);
  return true;
};
...
var toggleList = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor, tr: tr2, state, dispatch, chain, commands, can }) => {
  ...
  if (!keepMarks || !marks || !dispatch)
    return chain().command(() => {
      if (can().wrapInList(listType, attributes))
        return true;
      return commands.clearNodes();
...
var updateDecorations = (extensionName) => ({ tr: tr2, dispatch }) => {
  if (dispatch)
    tr2.setMeta(DECORATION_MANAGER_PLUGIN_KEY, {
      type: "force",
      name: extensionName
    });
  return true;
...
function findDuplicateWidgetKeys(decorationSet) {
  ...
  for (const decoration of decorationSet.find()) {
    ...
    const extensions2 = (_extensionsByKey$get = extensionsByKey.get(key)) !== null && _extensionsByKey$get !== void 0 ? _extensionsByKey$get : /* @__PURE__ */ new Set();
    extensions2.add(extension);
    extensionsByKey.set(key, extensions2);
    counts.set(key, ((_counts$get = counts.get(key)) !== null && _counts$get !== void 0 ? _counts$get : 0) + 1);
  }
  return Array.from(extensionsByKey, ([key, extensions2]) => ({
    key,
...
var DecorationManager = class {
  ...
  warnDuplicateWidgetKeys(state) {
    ...
    for (const { key, extensions: extensions2 } of duplicateKeys) {
      if (this.warnedWidgetKeys.has(key))
        continue;
      const names = Array.from(extensions2).map((name) => `"${name}"`).join(", ");
      console.warn(`[tiptap warn]: Duplicate widget decoration key "${key}" in extension${extensions2.size === 1 ? "" : "s"} ${names}. Widget decoration keys must be globally unique, otherwise ProseMirror misplaces the widget DOM. Use a stable, unique key (e.g. \`comment-\${id}\`).`);
    }
...
function run$1(config) {
  ...
  if ($from.parent.type.spec.code || !!((_ref = $from.nodeBefore || $from.nodeAfter) === null || _ref === void 0 ? void 0 : _ref.marks.find((mark) => mark.type.spec.code)))
    return false;
  let matched = false;
  ...
  rules.forEach((rule) => {
    ...
    if (!match2)
      return;
    const matchedDocLength = match2[0].length - text.length;
...
var Mark2 = class Mark3 extends Extendable {
  ...
  static handleExit({ editor, mark }) {
    ...
    if (currentPos.pos === currentPos.end()) {
      ...
      if (!!!currentMarks.find((m) => (m === null || m === void 0 ? void 0 : m.type.name) === mark.name))
        return false;
...
function run(config) {
  ...
  state.doc.nodesBetween(from2, to, (node, pos) => {
    ...
    if (((_node$type = node.type) === null || _node$type === void 0 || (_node$type = _node$type.spec) === null || _node$type === void 0 ? void 0 : _node$type.code) || !(node.isText || node.isTextblock || node.isInline))
      return;
    ...
    pasteRuleMatcherHandler(textToMatch, rule.find, pasteEvent).forEach((match2) => {
...
var ExtensionManager = class {
  ...
  setupExtensions() {
    ...
    this.editor.extensionStorage = Object.fromEntries(extensions2.map((extension) => [extension.name, extension.storage]));
    extensions2.forEach((extension) => {
      const context = {
        name: extension.name,
        options: extension.options,
      ...
      if (extension.type === "mark") {
        ...
        if ((_callOrReturn = callOrReturn(getExtensionField(extension, "keepOnSplit", context))) !== null && _callOrReturn !== void 0 ? _callOrReturn : true)
          this.splittableMarks.push(extension.name);
        if (!((_callOrReturn2 = callOrReturn(getExtensionField(extension, "clearable", context))) !== null && _callOrReturn2 !== void 0 ? _callOrReturn2 : true))
...
var Delete = Extension.create({
  ...
  onUpdate({ transaction, appendedTransactions }) {
    ...
    const callback = () => {
      ...
      if ((_this$editor$options$ = (_this$editor$options$2 = this.editor.options.coreExtensionOptions) === null || _this$editor$options$2 === void 0 || (_this$editor$options$2 = _this$editor$options$2.delete) === null || _this$editor$options$2 === void 0 || (_this$editor$options$3 = _this$editor$options$2.filterTransaction) === null || _this$editor$options$3 === void 0 ? void 0 : _this$editor$options$3.call(_this$editor$options$2, transaction)) !== null && _this$editor$options$ !== void 0 ? _this$editor$options$ : transaction.getMeta("y-sync$"))
        return;
    ...
    if ((_this$editor$options$4 = (_this$editor$options$5 = this.editor.options.coreExtensionOptions) === null || _this$editor$options$5 === void 0 || (_this$editor$options$5 = _this$editor$options$5.delete) === null || _this$editor$options$5 === void 0 ? void 0 : _this$editor$options$5.async) !== null && _this$editor$options$4 !== void 0 ? _this$editor$options$4 : true)
  ...
  addProseMirrorPlugins() {
    return [new Plugin({
      ...
      appendTransaction: (transactions, oldState, newState) => {
        ...
        if (empty2 || !(from2 === allFrom && to === allEnd))
          return;
        if (!isNodeEmpty(newState.doc))
          return;
  ...
  querySelectorAll(selector, attributes = {}, firstItemOnly = false) {
    ...
    this.children.forEach((childPos) => {
      ...
      if (childPos.node.type.name === selector) {
        if (attrKeys.every((key) => attributes[key] === childPos.node.attrs[key]))
          nodes.push(childPos);
      }
      if (firstItemOnly && nodes.length > 0)
        return;
...
var Editor = class extends EventEmitter {
  ...
  unmount() {
    if (this.editorView) {
      ...
      if (dom === null || dom === void 0 ? void 0 : dom.editor)
        delete dom.editor;
...
function nodeInputRule(config) {
  return new InputRule({
    ...
    handler: ({ state, range, match: match2 }) => {
      ...
      if (match2[1]) {
        let matchStart = start + match2[0].lastIndexOf(match2[1]);
        if (matchStart > end)
          matchStart = end;
        else
  ...
  return new PasteRule({
    ...
    handler: ({ state, range, match: match2, pasteEvent }) => {
      ...
      if (captureGroup) {
        ...
        tr2.addMark(range.from + startSpaces, markEnd, config.type.create(attributes || {}));
        if (!(match2.index !== void 0 && match2.input !== void 0 && match2.index + match2[0].length >= match2.input.length))
          tr2.removeStoredMark(config.type);
      }
    }
  });
      ...
      props: { decorations: ({ doc: doc3, selection }) => {
        ...
        doc3.descendants((node, pos) => {
          ...
          if (!(anchor >= pos && anchor <= pos + node.nodeSize - 1))
            return false;
          currentLevel += 1;
          if (this.options.mode === "deepest" && maxLevels - currentLevel > 0 || this.options.mode === "shallowest" && currentLevel > 1)
...
function collectBlocksForChange(doc3, change) {
  ...
  if (change.to > change.from)
    ranges.push(toContentRelativeRange(doc3, resolveTopLevelRange(doc3, Math.min(change.to, doc3.content.size + 1) - 1)));
  else if (change.from < doc3.content.size + 1)
    ranges.push(toContentRelativeRange(doc3, resolveTopLevelRange(doc3, Math.min(change.from + 1, doc3.content.size))));
  return ranges;
}
  ...
  addProseMirrorPlugins() {
    ...
    return [new Plugin({
      ...
      appendTransaction: (transactions, __, state) => {
        ...
        if (transactions.some((transaction) => transaction.getMeta("skipTrailingNode")))
          return;
...
var handleBackspace = (editor, type) => {
  ...
  if (previous.type !== type || !((_previous$lastChild = previous.lastChild) === null || _previous$lastChild === void 0 ? void 0 : _previous$lastChild.isTextblock))
    return false;
...
var inputRegexMatch = (text) => {
  ...
  if (!match2)
    return null;
  if (match2.index > 0 && text[match2.index - 1] === "`")
    return null;
  return {
    index: match2.index,
    text: match2[0],
...
var CodeBlock = Node2.create({
  ...
  parseMarkdown: (token, helpers) => {
    ...
    if (((_token$raw = token.raw) === null || _token$raw === void 0 ? void 0 : _token$raw.startsWith("```")) === false && ((_token$raw2 = token.raw) === null || _token$raw2 === void 0 ? void 0 : _token$raw2.startsWith("~~~")) === false && token.codeBlockStyle !== "indented")
      return [];
...
\`\`\``;
  ...
  addInputRules() {
    return [textblockTypeInputRule({
      ...
      type: this.type,
      getAttributes: (match2) => ({ language: match2[1] })
...
function isConvertibleLink(text, match2, isAllowedHref) {
  var _match$index, _match$index2;
  const [, linkText, href] = match2;
  if ((match2.index ? text[match2.index - 1] : void 0) === "!" || isEscaped(text, (_match$index = match2.index) !== null && _match$index !== void 0 ? _match$index : 0))
    return false;
  if (isInsideCodeSpan(text, (_match$index2 = match2.index) !== null && _match$index2 !== void 0 ? _match$index2 : 0))
    return false;
  return !!linkText.trim() && isAllowedHref(href);
}
function toRuleMatch(match2) {
  var _ref, _match$index3;
  const [linkSyntax, linkText, href, , straightQuotedTitle, curlyDoubleTitle, curlySingleTitle] = match2;
  ...
  return {
    index: (_match$index3 = match2.index) !== null && _match$index3 !== void 0 ? _match$index3 : 0,
    text: linkSyntax,
...
function getMarkdownLinkAttributes(match2) {
  var _match$data, _match$data$title, _match$data2;
  return {
    href: (_match$data = match2.data) === null || _match$data === void 0 ? void 0 : _match$data.href,
    title: (_match$data$title = (_match$data2 = match2.data) === null || _match$data2 === void 0 ? void 0 : _match$data2.title) !== null && _match$data$title !== void 0 ? _match$data$title : null
  };
}
function markdownLinkInputRule(config) {
  const rule = markInputRule({
    find: (text) => {
      const match2 = MARKDOWN_LINK_INPUT_REGEX.exec(text);
      if (!match2 || !isConvertibleLink(text, match2, config.isAllowedHref))
        return null;
      return toRuleMatch(match2);
    },
    type: config.type,
...
function markdownLinkPasteRule(config) {
  const rule = markPasteRule({
    find: (text) => {
      ...
      for (const match2 of text.matchAll(MARKDOWN_LINK_PASTE_REGEX))
        if (isConvertibleLink(text, match2, config.isAllowedHref))
          markdownMatches.push(toRuleMatch(match2));
  ...
  return new PasteRule({
    ...
    handler: (props) => {
      ...
      const result = rule.handler(props);
      if (result !== null && props.state.tr.steps.length && ((_props$match$data = props.match.data) === null || _props$match$data === void 0 ? void 0 : _props$match$data.markdown))
        props.state.tr.setMeta("preventAutolink", true);
      return result;
    }
  });
  ...
  addPasteRules() {
    ...
    return [markPasteRule({
      ...
      getAttributes: (match2) => {
        var _match$data;
        return { href: (_match$data = match2.data) === null || _match$data === void 0 ? void 0 : _match$data.href };
      }
    })];
  },
  addProseMirrorPlugins() {
...
var handleTab = (editor, name, parentListTypes) => {
  ...
  if (!$from.parent.isTextblock)
    return false;
  if (isNodeActive(state, name))
    return false;
  const previous = getPreviousBlockSibling($from);
  if (!previous || !parentListTypes.includes(previous.type.name))
    return false;
  const lastItem = previous.lastChild;
  if (!lastItem || lastItem.type.name !== name)
    return false;
...
function collectOrderedListItems(lines) {
  ...
  while (currentLineIndex < lines.length) {
    ...
    if (!match2)
      break;
    const [, indent, marker, _separator, content] = match2;
    const indentLevel = indent.length;
    const number3 = parseInt(marker, 10);
    const markerType = isNaN(number3) ? detectMarkerType(marker) : void 0;
    const itemNumber = isNaN(number3) ? markerToStart(marker) : number3;
    ...
    while (nextLineIndex < lines.length) {
      ...
      if (nextLine.match(ORDERED_LIST_ITEM_REGEX))
        break;
      if (nextLine.trim() === "") {
...
function parsePlainTextOrderedListPaste(text) {
  ...
  for (const line of lines) {
    const match2 = line.trim().match(PLAIN_TEXT_ORDERED_LIST_LINE_REGEX);
    if (!match2)
      return null;
    parsedItems.push({
      marker: match2[1],
...
function cssListStyleTypeToHtmlType(style2) {
  const match2 = style2.match(/list-style-type\s*:\s*([^;]+)/i);
  if (!match2)
    return null;
  switch (match2[1].trim().toLowerCase()) {
    case "upper-roman":
      return "I";
    case "lower-roman":
      return "i";
...
var OrderedList = Node2.create({
  ...
  addInputRules() {
    const joinPredicate = (match2, node) => {
      return (!node.attrs.type || node.attrs.type === "1") && node.childCount + node.attrs.start === +match2[1];
    };
    let inputRule = wrappingInputRule({
...
var TaskItem = Node2.create({
  ...
  addInputRules() {
    return [wrappingInputRule({
      find: inputRegex2,
      type: this.type,
      getAttributes: (match2) => ({ checked: match2[match2.length - 1] === "x" })
    })];
  }
});
...
var Paragraph = Node2.create({
  ...
  parseMarkdown: (token, helpers) => {
    ...
    if (tokens.length === 1 && tokens[0].type === "text" && (tokens[0].raw === EMPTY_PARAGRAPH_MARKDOWN || tokens[0].text === EMPTY_PARAGRAPH_MARKDOWN || tokens[0].raw === NBSP_CHAR || tokens[0].text === NBSP_CHAR) && content.length === 1 && content[0].type === "text" && (content[0].text === EMPTY_PARAGRAPH_MARKDOWN || content[0].text === NBSP_CHAR))
...
var StarterKit = Extension.create({
  ...
  addExtensions() {
    ...
    if (this.options.listKeymap !== false) {
      var _this$options;
      extensions2.push(ListKeymap.configure((_this$options = this.options) === null || _this$options === void 0 ? void 0 : _this$options.listKeymap));
    }
    if (this.options.link !== false) {
      var _this$options2;
      extensions2.push(Link.configure((_this$options2 = this.options) === null || _this$options2 === void 0 ? void 0 : _this$options2.link));
    }
    if (this.options.orderedList !== false)
      extensions2.push(OrderedList.configure(this.options.orderedList));
    ...
    if (this.options.text !== false)
      extensions2.push(Text2.configure(this.options.text));
    if (this.options.underline !== false) {
      var _this$options3;
      extensions2.push(Underline.configure((_this$options3 = this.options) === null || _this$options3 === void 0 ? void 0 : _this$options3.underline));
    }
    if (this.options.trailingNode !== false) {
      var _this$options4;
      extensions2.push(TrailingNode.configure((_this$options4 = this.options) === null || _this$options4 === void 0 ? void 0 : _this$options4.trailingNode));
    }
    return extensions2;
  }
});
...
function replace2(name, factory) {
  components[name] = factory;
...
function resolve(name) {
  ...
  return (extensions[name] ?? []).reduce((base2, wrap2) => wrap2(base2), factory);
}
