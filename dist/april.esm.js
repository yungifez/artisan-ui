...
var RangeModeHandler = class {
  constructor(selected, required, min, max) {
    ...
    if (selected?.from) {
      ...
      if (from2) this.dayClicked(from2);
    }
...
var calendar_default = (selected, mode, disabled, min, max, required, options = {}) => ({
  ...
  firstAllowedMonth() {
    ...
    const year = this.fromYear == null ? /* @__PURE__ */ new Date(-864e13) : new Date(this.fromYear, 0, 1);
  ...
  lastAllowedMonth() {
    ...
    const year = this.toYear == null ? /* @__PURE__ */ new Date(864e13) : new Date(this.toYear, 11, 1);
    return configured && configured < year ? new Date(configured.getFullYear(), configured.getMonth(), 1) : year;
  },
  isSelectedDay(day) {
  ...
  isToday(date) {
    const today = dateWithoutTime(/* @__PURE__ */ new Date());
...
var command_default = (value) => ({
  ...
  fuzzySearch(keyword, text) {
    ...
    let keywordIndex = 0;
    for (let i2 = 0; i2 < textLower.length; i2++) {
      if (textLower[i2] === keywordLower[keywordIndex]) {
        keywordIndex++;
      }
...
var toggleList = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor, tr: tr2, state, dispatch, chain, commands, can }) => {
  ...
  if ((isInsideExistingList || hasWholeDocSelectedList) && currentList) {
    ...
    if (isList(currentList.node.type.name, extensions2) && listType.validContent(currentList.node.content)) return chain().command(() => {
...
function run$1(config) {
  ...
  rules.forEach((rule) => {
    ...
    if (matchedDocLength > 0) {
      ...
      if (matchStartOffset < 0 || $from.parent.textBetween(matchStartOffset, $from.parentOffset) !== match2[0].slice(0, matchedDocLength)) return;
    }
  ...
  return new InputRule({
    ...
    handler: ({ state, range, match: match2 }) => {
      ...
      if (match2[1]) {
        ...
        else end = matchStart + match2[1].length;
...
function wrappingInputRule(config) {
  return new InputRule({
    ...
    handler: ({ state, range, match: match2, chain }) => {
      ...
      if (before && before.type === config.type && canJoin(tr2.doc, range.from - 1) && (!config.joinPredicate || config.joinPredicate(match2, before))) tr2.join(range.from - 1);
    },
    undoable: config.undoable
  });
}
  ...
  return new PasteRule({
    ...
    handler: ({ state, range, match: match2, pasteEvent }) => {
      ...
      if (captureGroup) {
        ...
        tr2.addMark(range.from + startSpaces, markEnd, config.type.create(attributes || {}));
        if (!(match2.index !== void 0 && match2.input !== void 0 && match2.index + match2[0].length >= match2.input.length)) tr2.removeStoredMark(config.type);
      }
    }
  });
}
...
var inputRegexMatch = (text) => {
  ...
  if (match2.index > 0 && text[match2.index - 1] === "`") return null;
  return {
    index: match2.index,
    text: match2[0],
...
var pasteRegexMatch = (text) => {
  ...
  while ((match2 = regex.exec(text)) !== null) {
    if (match2.index > 0 && text[match2.index - 1] === "`") continue;
    matches2.push({
      index: match2.index,
      text: match2[0],
      replaceWith: match2[1]
...
function isConvertibleLink(text, match2, isAllowedHref) {
  ...
  if ((match2.index ? text[match2.index - 1] : void 0) === "!" || isEscaped(text, (_match$index = match2.index) !== null && _match$index !== void 0 ? _match$index : 0)) return false;
  if (isInsideCodeSpan(text, (_match$index2 = match2.index) !== null && _match$index2 !== void 0 ? _match$index2 : 0)) return false;
...
function markdownLinkInputRule(config) {
  const rule = markInputRule({
    find: (text) => {
      ...
      if (!match2 || !isConvertibleLink(text, match2, config.isAllowedHref)) return null;
...
function markdownLinkPasteRule(config) {
  const rule = markPasteRule({
    find: (text) => {
      ...
      for (const match2 of text.matchAll(MARKDOWN_LINK_PASTE_REGEX)) if (isConvertibleLink(text, match2, config.isAllowedHref)) markdownMatches.push(toRuleMatch(match2));
...
var ListKit = Extension.create({
  ...
  addExtensions() {
    const extensions2 = [];
    if (this.options.bulletList !== false) extensions2.push(BulletList.configure(this.options.bulletList));
    if (this.options.listItem !== false) extensions2.push(ListItem.configure(this.options.listItem));
    if (this.options.listKeymap !== false) extensions2.push(ListKeymap.configure(this.options.listKeymap));
    if (this.options.orderedList !== false) extensions2.push(OrderedList.configure(this.options.orderedList));
    if (this.options.taskItem !== false) extensions2.push(TaskItem.configure(this.options.taskItem));
    if (this.options.taskList !== false) extensions2.push(TaskList.configure(this.options.taskList));
    return extensions2;
  }
});
...
var StarterKit = Extension.create({
  ...
  addExtensions() {
    const extensions2 = [];
    if (this.options.bold !== false) extensions2.push(Bold.configure(this.options.bold));
    if (this.options.blockquote !== false) extensions2.push(Blockquote.configure(this.options.blockquote));
    if (this.options.bulletList !== false) extensions2.push(BulletList.configure(this.options.bulletList));
    if (this.options.code !== false) extensions2.push(Code.configure(this.options.code));
    if (this.options.codeBlock !== false) extensions2.push(CodeBlock.configure(this.options.codeBlock));
    if (this.options.document !== false) extensions2.push(Document.configure(this.options.document));
    if (this.options.dropcursor !== false) extensions2.push(Dropcursor.configure(this.options.dropcursor));
    if (this.options.gapcursor !== false) extensions2.push(Gapcursor.configure(this.options.gapcursor));
    if (this.options.hardBreak !== false) extensions2.push(HardBreak.configure(this.options.hardBreak));
    if (this.options.heading !== false) extensions2.push(Heading.configure(this.options.heading));
    if (this.options.undoRedo !== false) extensions2.push(UndoRedo.configure(this.options.undoRedo));
    if (this.options.horizontalRule !== false) extensions2.push(HorizontalRule.configure(this.options.horizontalRule));
    if (this.options.italic !== false) extensions2.push(Italic.configure(this.options.italic));
    if (this.options.listItem !== false) extensions2.push(ListItem.configure(this.options.listItem));
    ...
    if (this.options.orderedList !== false) extensions2.push(OrderedList.configure(this.options.orderedList));
    if (this.options.paragraph !== false) extensions2.push(Paragraph.configure(this.options.paragraph));
    if (this.options.strike !== false) extensions2.push(Strike.configure(this.options.strike));
    if (this.options.text !== false) extensions2.push(Text2.configure(this.options.text));
...
var select_default = (multiple, disabled) => ({
    ...
    if (!this.options[index].selected || !this.multiple) {
      if (!this.multiple) {
        for (let i2 = 0; i2 < this.selected.length; i2++) {
          this.options[this.selected[i2]].selected = false;
        }
  ...
  loadOptions() {
    ...
    for (let i2 = 0; i2 < options.length; i2++) {
      this.options.push({
        value: options[i2].value,
        text: options[i2].innerText,
        disabled: options[i2].disabled,
        selected: options[i2].getAttribute("selected") != null || i2 == 0 && !this.multiple ? this.selected.push(i2) : false
      });
      if (!this.multiple && options[i2].getAttribute("selected") != null) {
